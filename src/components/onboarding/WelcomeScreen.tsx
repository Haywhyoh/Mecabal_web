...
export default function WelcomeScreen() {
  ...
  const handleGoogleButtonClick = useCallback(async () => {
    try {
      ...
      // The callback from initializeGoogleSignIn will handle the response
      window.google.accounts.id.prompt((notification: any) => {
        if (notification.isNotDisplayed()) {
          // One Tap not displayed - user might need to sign in manually
          // Try to show the popup flow instead
          console.log('One Tap not displayed, attempting popup flow');
          setIsGoogleLoading(false);
